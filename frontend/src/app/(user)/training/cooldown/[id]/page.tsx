...
import {
  useCooldownTrainings,
  useFlexibilityCheck,
} from '@/hooks/useFlexibilityChecks';

export default function CooldownDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();

  // 一覧（クールダウン全体）を取得して、分母・前後の種目を計算する
  const {
    trainings,
  ...
  const handleTouchEnd = (
    ...
    if (deltaX > threshold && prevId !== null) {
      router.push(`/training/cooldown/${prevId}`);
    } else if (deltaX < -threshold && nextId !== null) {
      router.push(`/training/cooldown/${nextId}`);
    }
    ...
    <div className="container mx-auto p-4">
      ...
      >
        ...
        <section className="mt-4 p-4 bg-white rounded-lg border border-gray-100">
          ...
          <p className="mb-2 text-xs text-gray-600">
            このクールダウンに取り組んだことを記録します（評価は後でコーチがつけます）。
          </p>
          <textarea
      ...
      <div className="mt-6 flex items-center justify-between">
        <button
          ...
          onClick={() => prevId !== null && router.push(`/training/cooldown/${prevId}`)}
        ...
        <button
          className="rounded border border-gray-300 px-3 py-2 text-sm disabled:opacity-40"
          onClick={() => nextId !== null && router.push(`/training/cooldown/${nextId}`)}
