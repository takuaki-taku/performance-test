...
export default function WarmupDetailPage() {
  const { id } = useParams<{ id: string }>();
  const router = useRouter();

  // 一覧（ウォームアップ全体）を取得して、分母・前後の種目を計算する
  const {
    trainings,
  ...
  // デバッグログ
  useEffect(() => {
    console.log('[WarmupDetailPage] フィードバック状態:', {
      todayResultId,
  ...
  useEffect(() => {
    if (!training || !userId) {
      console.log('[WarmupDetailPage] トレーニング結果取得スキップ:', { training: !!training, userId });
      return;
    }
    ...
    const fetchTodayResult = async () => {
      try {
        ...
        const url = `${apiBase}/user-training-results/${userId}/${training.id}`;
        console.log('[WarmupDetailPage] トレーニング結果取得開始:', { userId, trainingId: training.id, today, url });
        const response = await axios.get(url);
        console.log('[WarmupDetailPage] トレーニング結果取得成功:', {
          userId,
          ...
          resultCount: response.data?.length || 0
            ...
            console.log('[WarmupDetailPage] 今日の結果検索:', { today, todayResult });
            if (todayResult) {
              console.log('[WarmupDetailPage] 今日の結果IDを設定:', todayResult.id);
              ...
              console.log('[WarmupDetailPage] 今日の結果なし、最新の結果IDを使用:', latestResult.id, latestResult.date);
              setTodayResultId(latestResult.id);
            } else {
              console.log('[WarmupDetailPage] 結果が見つかりませんでした');
              setTodayResultId(null);
            }
      } catch (err: any) {
        console.error('[WarmupDetailPage] トレーニング結果取得エラー:', {
          userId,
          trainingId: training.id,
          error: err,
          message: err.message,
  ...
  const handleTouchEnd = (
    ...
    if (deltaX > threshold && prevId !== null) {
      router.push(`/training/warmup/${prevId}`);
    } else if (deltaX < -threshold && nextId !== null) {
      router.push(`/training/warmup/${nextId}`);
    }
    ...
    return (
      <div className="p-4">
        ...
        <button
          ...
          onClick={() => router.push('/training/warmup')}
        >
          一覧に戻る
        </button>
      </div>
    ...
    <div className="container mx-auto p-4">
        ...
        {!todayResultId && (
          <section className="mt-4 p-4 bg-white rounded-lg border border-gray-100">
            ...
            <p className="mb-2 text-xs text-gray-600">
              このウォームアップに取り組んだことを記録します（評価は後でコーチがつけます）。
            </p>
            <textarea
      ...
      <div className="mt-6 flex items-center justify-between">
        <button
          ...
          onClick={() => prevId !== null && router.push(`/training/warmup/${prevId}`)}
        ...
        <button
          className="rounded border border-gray-300 px-3 py-2 text-sm disabled:opacity-40"
          onClick={() => nextId !== null && router.push(`/training/warmup/${nextId}`)}
